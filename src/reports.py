...
def add_file(report_id, file_data, drive_file_id=None, persist_content=True, action_id=None):
    kind, local_path, original_filename, mime_type, file_size, content = file_data
    with connection() as conn:
        conn.execute("""INSERT INTO report_files
                      (report_id, file_type, local_path, original_filename, mime_type, file_size, content, drive_file_id, action_id)
                      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                     (report_id, kind, local_path, original_filename, mime_type, file_size,
                      content if persist_content else None, drive_file_id, action_id))
